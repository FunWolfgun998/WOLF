...
void ASTPrinter::operator()(const std::unique_ptr<BinaryExpr>& node) {
    ...
    printBranch(node->right.as, true, "Right");
}
...
void ASTPrinter::operator()(const std::unique_ptr<CallExpr>& node) {
    ...
    printBranch(node->callee.as, noArgs, "Callee");

    // Visit all arguments
    for (size_t i = 0; i < node->arguments.size(); ++i) {
        bool isLast = (i == node->arguments.size() - 1);
        printBranch(node->arguments[i].as, isLast, "Arg[" + std::to_string(i) + "]");
    }
}
...
void ASTPrinter::operator()(const std::unique_ptr<ArrayLiteralExpr>& node) {
    ...
    for (size_t i = 0; i < node->elements.size(); ++i) {
        ...
        printBranch(node->elements[i].as, isLast, label);
    }
}
...
void ASTPrinter::operator()(const std::unique_ptr<VarDeclStmt>& node) {
    ...
    if (node->initializer != nullptr) {
        printBranch(node->initializer->as, true, "Initializer");
    }
}
...
void ASTPrinter::operator()(const std::unique_ptr<IfStmt>& node) {
    ...
    //Is the last branch if there are no elifs and else
    printBranch(node->thenBranch.as, !(hasElifs || hasElse), "Then");
    ...
    for(size_t j = 0; j < node->elifBranches.size(); ++j) {
        ...
        printBranch(node->elifBranches[j].condition.as, false, "Condition");
        printBranch(node->elifBranches[j].block.as, true, "Body");
    ...
    // 4. ELSE Branch
    if (hasElse) {
        printBranch(node->elseBranch->as, true, "Else");
    }
}
...
void ASTPrinter::operator()(const std::unique_ptr<BreakStmt>& node) {
    out << "BreakStmt\n";
}

void ASTPrinter::operator()(const std::unique_ptr<ContinueStmt>& node) {
    out << "ContinueStmt\n";
}
...
void ASTPrinter::operator()(const std::unique_ptr<StructDeclStmt>& node) {
    ...
    if (node->fields.empty()) {
        ...
        for (size_t i = 0; i < node->fields.size(); ++i) {
            ...
            printBranch(node->fields[i].as, isLast, "Field[" + std::to_string(i) + "]");
        }
    }
}